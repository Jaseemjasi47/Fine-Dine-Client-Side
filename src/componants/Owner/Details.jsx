...
function Details({ handleShowBookings }) {
  ...
  const fetchRestaurantsData = async () => {
    ...
    try {
      const response = await axiosInstance.get(`Restaurants/user_restaurants/${user.email}/`);
  ...
  return (
    <div className='admin-content bg-white'>
      {editingRestaurantId ? (
        ...
        <>
      <h1 className='text-center text-uppercase mt-3'>Restaurants</h1>
      <div id="tab-1" className="tab-pane fade show p-0 restaurant-container">
            ...
            restaurants.map((restaurant) => (
              <div key={restaurant.id} className="col-lg-6">
                <div className="w-100 d-flex flex-column text-start ps-4">
                  ...
                  <small className="fst-italic">Location : {restaurant.place}</small>
                </div>
                ...
                <div className="btn btn-primary my-2" onClick={() => handleBookingsClick(restaurant.id)}>
                Bookings
                </div>
                <div className="btn border my-2 float-end mr-2" onClick={() => handleEditClick(restaurant.id)}>
                Edit
                </div>
              </div>
            ))
